		...
		case __('Add File', 'wp-downloadmanager'):
			...
			switch($file_type) {
				case 0:
					...
					$file_size = filesize($file_path.iconv("utf-8","gb2312",$file));
					break;
				case 1:
					if($_FILES['file_upload']['size'] > get_max_upload_size()) {
						...
						if(is_uploaded_file($_FILES['file_upload']['tmp_name'])) {
							...
							if(move_uploaded_file($_FILES['file_upload']['tmp_name'], //$file_path.$file_upload_to.getFileBaseName($_FILES['file_upload']['name']))) 
							//下面这行为了支持中文文件上传名称在服务器端显示正确
								$file_path.$file_upload_to.iconv("utf-8","gb2312",getFileBaseName($_FILES['file_upload']['name']))))
							{
								$file = $file_upload_to.getFileBaseName($_FILES['file_upload']['name']);
								$file = download_rename_file($file_path,$file);//读取的是服务器上的文件名称
								$file_size = filesize($file_path.iconv("utf-8","gb2312",$file));	//读取的是服务器上的文件体积大小
							} else {
			...
			if(empty($file_name)) {
				$file_name = getFileBaseName($file);
			}
