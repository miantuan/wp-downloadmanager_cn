...
function download_file() {
	...
	if($dl_id > 0 || !empty($dl_name)) {
		if($dl_id > 0 && $download_options['use_filename'] == 0) {
			...
			$file = $wpdb->get_row("SELECT file_id, file, file_permission FROM $wpdb->downloads WHERE file_name = \"$dl_name\" AND file_permission != -2");
		}
		if(!$file) {
		...
		$file_name = stripslashes($file->file);//把文件的路径作为文件名称
...
echo $file_path.$file_name;
				...
				if(true || $download_method == 0) {
					...
					header("Content-Disposition: attachment; filename=".getFileBaseName($file_name).";");
				...
				if(false && ini_get('allow_url_fopen') && $download_method == 0) {
					header("Pragma: public");
					header("Expires: 0");
					header("Cache-Control: must-revalidate, post-check=0, pre-check=0"); 
					header("Content-Type: application/force-download");
...
//function download_file_url($file_id, $file_name,$file_url) 
	function download_file_url($file_id, $file_name) 
	{
	$file_id = intval($file_id);
	if (!is_remote_file($file_name)) $file_name = substr(stripslashes($file_name), 1);
	$download_options = get_option('download_options');
	...
	if($download_nice_permalink == 1) {
		if($download_use_filename == 1) {
			...
			//$download_file_url = get_option('home').'/download'.$file_url;
			...
			$download_file_url = $file_name;
			}

/*
			if (!is_remote_file($file_url))
			{
			$download_file_url = get_option('home').'/download'.$file_url;
			}
			else {
...
function download_shortcode($atts) {
	extract(shortcode_atts(array('id' => '0', 'category' => '0', 'display' => 'both', 'name' => '0', 'path' => '0'), $atts));
	if(!is_feed()) {
		$conditions = array();
		if($id != '0') {
			if(strpos($id, ',') !== false) {
...
function downloads_page($category_id = 0) {
	...
	if($files) {
		...
		foreach($files as $file) {
			...
			//$template_download_listing = str_replace("%FILE_DOWNLOAD_URL%", download_file_url($file->file_id, $file->file_name), $template_download_listing);
	...
	if($download_files) {
		foreach($download_files as $download_file) {
			if($download_file == $selected) {
				echo '<option value="'.iconv("gb2312","utf-8",$download_file).'" selected="selected">'.iconv("gb2312","utf-8",$download_file).'</option>'."\n";	
			} else {
				echo '<option value="'.iconv("gb2312","utf-8",$download_file).'">'.iconv("gb2312","utf-8",$download_file).'</option>'."\n";	
			}
		}
	}
	if($download_files_subfolder) {
		foreach($download_files_subfolder as $download_file_subfolder) {
			if($download_file_subfolder == $selected) {
				echo '<option value="'.iconv("gb2312","utf-8",$download_file_subfolder).'" selected="selected">'.iconv("gb2312","utf-8",$download_file_subfolder).'</option>'."\n";	
			} else {
				echo '<option value="'.iconv("gb2312","utf-8",$download_file_subfolder).'">'.iconv("gb2312","utf-8",$download_file_subfolder).'</option>'."\n";	
			}
		}
	}
}
...
function download_rename_file($file_path, $file) {
	$rename = false;
	$file_old = iconv("utf-8","gb2312",$file);
	$file=iconv("utf-8","gb2312",$file);
	$file = str_replace(' ', '_', $file);
	$file = preg_replace('/[^A-Za-z0-9\-._\/\x7f-\xff]/', '', $file);
	if($file != $file_old) {
		$rename = rename($file_path.$file_old,$file_path.$file);//更新服务器上文件的的文件名称
	}
	if($rename) {
		return iconv("gb2312","utf-8",$file);
	} else {
		return  iconv("gb2312","utf-8",$file_old);
	}
}
...
function download_embedded($condition = '', $display = 'both') {
	...
	if($files) {
		...
		foreach($files as $file) {
			...
			//$template_download_embedded = str_replace("%FILE_DOWNLOAD_URL%", download_file_url($file->file_id, $file->file_name), $template_download_embedded);	
	...
	function get_most_downloaded($limit = 10, $chars = 0, $display = true) {
		...
		if($files) {
			...
			foreach($files as $file) {
				...
				//$template_download_most = str_replace("%FILE_DOWNLOAD_URL%", download_file_url($file->file_id, $file->file_name), $template_download_most);
